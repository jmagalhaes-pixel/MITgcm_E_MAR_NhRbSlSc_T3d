C $Header: /u/gcmpack/MITgcm/pkg/flt/flt_exchg.F,v 1.12 2009/02/13 04:22:22 jmc Exp $
...
C     == local variables ==
      ...
      Real*8 ilo, ihi, jlo, jhi, iNew, jNew
      PARAMETER(imax=9)
      PARAMETER(imax2=imax*max_npart_exch)
      CHARACTER*(MAX_LEN_MBUF) msgBuf

C     buffer for sending/receiving variables (E/W are also used for S/N)
      Real*8 fltbuf_sendE(imax2,nSx,nSy)
      Real*8 fltbuf_sendW(imax2,nSx,nSy)
      Real*8 fltbuf_recvE(imax2,nSx,nSy)
      Real*8 fltbuf_recvW(imax2,nSx,nSy)
...
C Remove from this tile-list, floats which have been sent to an other tile
             ...
             DO jp = npNew+1,npart_tile(bi,bj)
              IF ( npart(jp,bi,bj).NE.0.d0 ) THEN
                jl = jl + 1
                ip = deleteList(jl)
C        copy: ip <-- jp
