...
C $Name:  $
C modified by Maarten Buijsman, GFDL, 2010-12-7
C to have a different stretching that has infinite timescale or zero factor at
C the inner sponge boundary
C use linear first and only for west and east boundaries for u, v, and temp
C THUS not for salt and north and south boundaries
...
C     Loop counters
      ...
      Real*8 Urelaxobcsbound2,Vrelaxobcsbound2
...
C Northern Open Boundary
     ...
     &    - maskW(i,j,kLev,bi,bj) * lambda_obcs_u
...
Cmcb      write(*,*) 'in sponge .......888888888888888.'
Cmcb      write(*,*) spongeThickness,Urelaxobcsbound
      DO j=jMin,jMax
...
Cmcb first linear sponge
          Urelaxobcsbound2 = 1.d0 / Urelaxobcsbound
          lambda_obcs_u = 
     &        float(spongeThickness-isl+1)*Urelaxobcsbound2    
     &      / float(spongeThickness)

Cmcb then quadratic increase
          lambda_obcs_u = 
     &        Urelaxobcsbound2 * (float(spongeThickness-isl+1)    
     &      / float(spongeThickness))**2
...
C Northern Open Boundary
     ...
     &    - maskS(i,j,kLev,bi,bj) * lambda_obcs_v
