C $Header: /u/gcmpack/MITgcm/pkg/obcs/obcs_apply_sloc.F,v 1.4 2009/10/01 20:54:34 jmc Exp $
...
C Northern boundary
       J_obc = OB_Jn(I,bi,bj)
       IF (J_obc.NE.0) THEN
        obc_mask = maskS(I,J_obc,K,bi,bj)
...
C Southern boundary
       J_obc = OB_Js(I,bi,bj)
       IF (J_obc.NE.0) THEN
        obc_mask = maskS(I,J_obc+1,K,bi,bj)
...
C Eastern boundary
       I_obc = OB_Ie(J,bi,bj)
       IF (I_obc.NE.0) THEN
        obc_mask = maskW(I_obc,J,K,bi,bj)
...
C Western boundary
       I_obc=OB_Iw(J,bi,bj)
       IF (I_obc.NE.0) THEN
        obc_mask = maskW(I_obc+1,J,K,bi,bj)
