...
/**
 * Current version of this API, stored in libimalloc.dll
 * as i_version_dll and as i_version in all other cases.
 * This version number will be incremented when adding
 * features.
 *
 * Note that the initial releases of libimalloc.dll did not
 * have a version yet: they are assigned version 0.
 */
#define I_MALLOC_VERSION 1

/**
 * Minimum version that the current API is still compatible with,
 * stored in libimalloc.dll as i_min_version_dll and as i_min_version
 * in all other cases. This version number will be bumped to
 * I_MALLOC_VERSION when changing the API in a backwards-incompatible
 * way; this will be avoided at all costs.
 * 
 * If and only if i_min_version_dll <= I_MALLOC_VERSION is the DLL
 * compatible with code compiled against an older API specification.
 */
#define I_MALLOC_MIN_VERSION 0
...
extern INTEL_API_DEF int i_version_dll;
extern INTEL_API_DEF int i_min_version_dll;
...
extern int i_version;
extern int i_min_version;
