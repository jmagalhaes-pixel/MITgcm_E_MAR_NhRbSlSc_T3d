...
CEOP
     ...
     &     PPdumpFreq, PPtaveFreq
      ...
      Real*8 PPeps (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C----------------------------------------------------------------
      ...
      Real*8 PPviscArtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 PPdiffKrtave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8 PPepstave  (1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
...
C     == Local variables ==
      ...
      INTEGER bi, bj, K, iMin,iMax,jMin,jMax, I, J
      Real*8 DDTT
CEOP
...
C         transfer analyse a particular snap-shots
     ...
     &       myIter,myThid)
C        CALL WRITE_FLD_XYZ_RL('PPdiffKr.',suff,PPdiffKr,
C     &       myIter,myThid)
       endif
...
C        CALL TIMEAVE_RESET(PPdiffKrtave,Nr,bi,bj,myThid)
         CALL TIMEAVE_RESET(PPepstave,Nr,bi,bj,myThid)
         DO k=1,Nr
...
C     Time Average PP fields
     ...
     &       PPviscArtave, PPviscAr, Nr,DDTT,bi,bj,myThid)
        CALL TIMEAVE_CUMULATE(
     &       PPepstave,PPeps,Nr,DDTT,bi,bj,myThid)
...
C     Normalize by integrated time
     ...
     &        Nr, bi,bj,myThid)
         CALL TIMEAVE_NORMALIZ(PPepstave,pp_timeave,
     &        Nr, bi,bj,myThid)
C         CALL TIMEAVE_NORMALIZ(PPdiffKrtave,pp_timeave,
C     &        Nr, bi,bj,myThid)
        ENDDO
       ENDDO
...
C     transfer analyse a particular snap-shots
     ...
     &      myIter,myThid)
       CALL WRITE_FLD_XYZ_RL('PPeps-T.',suff,PPepstave,
     &      myIter,myThid)
C       CALL WRITE_FLD_XYZ_RL('PPdiffKr-T.',suff,PPdiffKrTave,
C     &      myIter,myThid)
...
C     &        PPdiffKrtave,PPdiffKr,Nr,DDTT,bi,bj,myThid)
C     Keep record of how much time has been integrated over
         DO k=1,Nr
          pp_TimeAve(k,bi,bj)=pp_TimeAve(k,bi,bj)+DDTT
         ENDDO
