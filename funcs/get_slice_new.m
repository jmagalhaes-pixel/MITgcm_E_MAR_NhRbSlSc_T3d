%% function [xl,yl,sl,varstruct] = get_slice_new(XC,YC,ZZ,xlp,ylp,nump,getpoints,scfac);
%% MCB, GFDL, 2012-03-19
...
function [xl,yl,sl,zl] = get_slice_mitgcm(XC,YC,ZZ,xlp,ylp,nump,getpoints,scfac);
...
%% limit data ===============================================
%DOFF = max([XC(1,2)-XC(1,1) YC(2,1)-YC(1,1)]);  %% max grid size
DOFF = max([abs(diff(xl([end 1]))) abs(diff(yl([end 1])))])/2;
Ix = find(XC(1,:)>minx-DOFF & XC(1,:)<maxx+DOFF );
Iy = find(YC(:,1)>miny-DOFF & YC(:,1)<maxy+DOFF );

%% grid ===============================================
zl = griddata(XC(Iy,Ix),YC(Iy,Ix),ZZ(Iy,Ix),xl,yl);
