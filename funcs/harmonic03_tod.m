%%---------------------------------------------------------------------------------------------------
%%-- function: [Ao,a,b,fq2,coef_det,sigma,fit] = harmonic03_tod(xt,xv,freq_sel,cel_num,plot_fig,freq_dat)
...
%%-- 2015-10-28 omitted 'data' from output
%%---------------------------------------------------------------------------------------------------
function [Ao,a,b,fq2,coef_det,sigma,fit]=harmonic03_tod(xt,xv,freq_sel,cel_num,plot_fig,freq_dat)
...
a   = zeros(nbins,length(freq_sel))*NaN; b=a;
...
for x4 = 1:nbins;	     %%-- Loop over all bins (Harmonic analysis for all depths)
    ...
    if length(xv2) > ntide;	%% Check if number of data points is smaller than number of a's and b's (too few => not a good fit possible)
        ...
        a(x4,1:(ntide-1)/2) = NaN;	%% a-value in acos(fq*t)
        b(x4,1:(ntide-1)/2) = NaN;	%% b-value in bsin(fq*t)
        coef_det(x4)        = NaN;
        sigma(x4)           = NaN;        
    end;
    clear x5 xt2 xv2 ;
